function autoMatch(transactie, lasten, periode) {
  // Stap 1: IBAN-matching — heeft voorrang op alle andere regels
  if (transactie.tegenrekening) {
    ...
    if (ibanLasten.length > 1) {
      // Meerdere lasten op zelfde IBAN: kies op bedrag (binnen 2 cent)
      const match = ibanLasten.find(l => Math.abs(Math.abs(transactie.bedrag) - l.bedrag) < 0.02);
      return match ? match.id : ibanLasten[0].id;
    }
    // Geen IBAN-match gevonden — val terug op regels 2 en 3
  }

  // Stap 2: Omschrijving patroon
  ...
  // Stap 3: Bedrag + datumvenster (alleen als geen IBAN gevonden)
  for (const last of lasten) {
